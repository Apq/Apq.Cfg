...
/// <summary>
/// Zookeeper 配置源测试
/// 注意：这些测试需要配置 Zookeeper 连接字符串才能运行
/// </summary>
public class ZookeeperCfgTests : IAsyncLifetime
{
    private ICfgRoot? _cfg;
    private readonly string _rootPath;

    public ZookeeperCfgTests()
    {
        _rootPath = $"/apqcfgtest/{Guid.NewGuid():N}";
    }

    public Task InitializeAsync()
    {
        if (!TestSettings.IsZookeeperConfigured)
        {
    ...
    [SkippableFact]
    public void Zookeeper_OverridesJson_WhenHigherLevel()
    {
        Skip.If(!TestSettings.IsZookeeperConfigured, "Zookeeper 连接字符串未配置，跳过测试");
