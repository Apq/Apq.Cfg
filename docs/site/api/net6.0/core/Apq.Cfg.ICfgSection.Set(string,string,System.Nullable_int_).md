#### [Apq\.Cfg](index.md 'index')
### [Apq\.Cfg](Apq.Cfg.md 'Apq\.Cfg').[ICfgSection](Apq.Cfg.ICfgSection.md 'Apq\.Cfg\.ICfgSection')

## ICfgSection\.Set\(string, string, Nullable\<int\>\) Method

设置配置值

```csharp
void Set(string key, string? value, System.Nullable<int> targetLevel=null);
```
#### Parameters

<a name='Apq.Cfg.ICfgSection.Set(string,string,System.Nullable_int_).key'></a>

`key` [System\.String](https://learn.microsoft.com/en-us/dotnet/api/system.string 'System\.String')

相对于此节的键名

<a name='Apq.Cfg.ICfgSection.Set(string,string,System.Nullable_int_).value'></a>

`value` [System\.String](https://learn.microsoft.com/en-us/dotnet/api/system.string 'System\.String')

配置值

<a name='Apq.Cfg.ICfgSection.Set(string,string,System.Nullable_int_).targetLevel'></a>

`targetLevel` [System\.Nullable&lt;](https://learn.microsoft.com/en-us/dotnet/api/system.nullable-1 'System\.Nullable\`1')[System\.Int32](https://learn.microsoft.com/en-us/dotnet/api/system.int32 'System\.Int32')[&gt;](https://learn.microsoft.com/en-us/dotnet/api/system.nullable-1 'System\.Nullable\`1')

目标层级，为null时使用可写的最高层级

### Example

```csharp
var dbSection = cfg.GetSection("Database");
dbSection.Set("Timeout", "60"); // 等同于 cfg.Set("Database:Timeout", "60")
```