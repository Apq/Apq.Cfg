...
var cfg = new CfgBuilder()
    // 基础配置不需要热重载
    .AddJson("appsettings.json", level: 0, reloadOnChange: false)
    // 只对需要动态更新的配置启用热重载
...
var cfg = new CfgBuilder()
    ...
    .AddJson($"appsettings.{env}.json", level: 1)    // 环境配置
...
// 1. 延迟加载远程配置
var cfg = new CfgBuilder()
    .AddJson("appsettings.json", level: 0)  // 本地配置先加载
    .Build();
