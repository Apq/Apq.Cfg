...
1. **Nacos 缺少热重载**：当前 Nacos 不支持热重载，可考虑实现长轮询机制
2. **Apollo 只读**：Apollo 不支持写入，建议在 API 层明确区分只读源
3. **缺少配置加密**：敏感配置明文存储存在安全风险
...
建议优先实现**配置加密**和**Zookeeper/Vault 扩展**，以满足企业级安全和多配置中心需求。
