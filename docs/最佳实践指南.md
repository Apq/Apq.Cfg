...
本文档提供了使用 Apq.Cfg 进行配置管理的最佳实践，帮助开发者构建更加健壮、安全和可维护的配置系统。
...
var cfg = new CfgBuilder()
    ...
    // 1层：环境特定默认值
    .AddJson($"appsettings.{Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") ?? "Production"}.json", 
...
var cfg = new CfgBuilder()
    .AddJson("appsettings.json", level: 0)
    .AddJson("appsettings.encrypted.json", level: 2, 
...
```csharp
// 生产环境配置文件权限设置
// - appsettings.json: 只读 (应用程序服务账户)
// - appsettings.Production.json: 只读 (配置管理服务账户)
// - appsettings.Development.json: 读写 (开发者账户)
```
...
遵循这些最佳实践可以帮助您构建更加健壮、安全和可维护的配置系统。记住，良好的配置管理是应用程序稳定运行的基础，值得投入时间和精力来正确实施。
